...
const eslint = (files, { nodeEnv } = {}) =>
  withLocalTmpDir(async () => {
    ...
    try {
      await execa('eslint', ['--ext', '.js,.json', '.'], {
        all: true,
...
export default {
  'dev dependency in root': async () =>
    expect(
      ...
      "error  'foo' should be listed in the project's dependencies, not devDependencies"
    ),
  'dev dependency in source': async () =>
    expect(
      await eslint({
        'node_modules/foo/index.js': 'export default 1',
        'package.json': endent`
  ...
  'restricted import: inside': async () =>
    expect(
      await eslint({
  ...
  'restricted import: outside': async () =>
    expect(
      await eslint({
        'test.js': endent`
        import puppeteer from 'puppeteer'
  ...
  'quotes: unnecessary escapes': async () =>
    expect(
      await eslint({
        'test.js': endent`
        export default 'foo \\'bar\\''
